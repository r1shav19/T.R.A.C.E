    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body:JSON.stringify({
          ...
          system:`You are T.R.A.C.E. crisis AI. Return ONLY valid JSON with keys: riskLevel (CRITICAL|HIGH|MODERATE), approachStrategy (string), searchPriority (array of 3 strings), doList (array of 3 strings), dontList (array of 2 strings), languageTip (string), medicalNote (string). No markdown, no backticks.`,
          messages:[{role:"user",content:`Generate field briefing: Name:${incident.profile.name}, Age:${incident.profile.age}, Condition:${incident.profile.condition}, Behavior:${incident.profile.behavior}, Language:${incident.profile.language}, Medical risk:${incident.profile.medicalRisk}%`}]
        })
      });
      ...
      setGeminiError("Using simulated Gemini profile for demo.");
      setGeminiProfile({
